...
def authenticate(username, password):
    ...
    if (obj is not None and obj != "" and obj["password"] == password
            and obj["enable"] == 1):
        encoded = jwt.encode(
            {
                "username":
                username,
                "exp":
                datetime.datetime.utcnow() + datetime.timedelta(seconds=14400),
            },
            config.getConf("jwtsecret"),
            algorithm="HS256",
        )
...
def Manageauthenticate(username, password):
    ...
    if (obj is not None and obj["password"] == hashlib.sha256(
