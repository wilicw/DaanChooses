    ...
    methods: {
      ...
      saveChoose: function (index, id) {
        if (id == -1) {
          ...
          //set club name
          let club = _.findLast(this.allChoose, obj => {
            return obj.id == id
          })
        ...
        //clean old choose
        _.map(this.availableChooses, obj => {
          if (obj.selected == index && obj.id != id)
            obj.selected = -1
          return obj
        })
      ...
      submit: async function (noPopUpMsg=true) {
        ...
        self.noFullError = _.findKey(self.alreadyChosen, ['club_id', -1])
        if (self.noFullError && !noPopUpMsg) {
